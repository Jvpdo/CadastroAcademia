...
app.put('/alunos/:id', protegerRota, async (req, res) => {
    ...
    try {
        ...
        // 1. Prepara a base da query e os valores
        let sql = `UPDATE alunos SET nome = ?, email = ?, telefone = ?, sexo = ?, dataNascimento = ?, faixa = ?, grau = ?, plano = ?`;
        let values = [nome, email, telefone, sexo, dataNascimento, faixa, grau, plano];

        // 2. Se uma nova senha foi fornecida, adiciona à query e aos valores
        if (senha && senha.trim() !== '') {
            if (senha.length < 6) {
...
app.get('/api/biblioteca', protegerRota, async (req, res) => {
    ...
    app.post('/api/me/verify-password', protegerRota, async (req, res) => {
    ...
    try {
        ...
        // 2. Compara a senha enviada com a que está no banco
        const senhaValida = await bcrypt.compare(senha, aluno.senha);
        if (!senhaValida) {
            return res.status(401).json({ error: 'A senha está incorreta.' });
        }

        // 3. Se a senha estiver correta, retorna sucesso
