...
export default function GerenciarHorariosScreen() {
  ...
  // A função fetchData agora busca tanto as aulas quanto a grade de horários
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      const [dataAulas, dataGrade] = await Promise.all([
        api.getHorarios(session),
        api.getGradeHorarios(session), // Busca os horários fixos
      ]);
      ...
      setHorariosFixos(dataGrade); // Preenche a grade com os dados do banco
    } catch (error: any) { Alert.alert('Erro', error.message || 'Não foi possível carregar os dados.'); } 
    finally { setIsLoading(false); }
  }, [session]);

  useEffect(() => { fetchData(); }, [fetchData]);

  // --- Funções do Modal de AULA (sem alterações) ---
  ...
  const handleSaveAula = async () => {
    ...
    const horarioData = {
      ...
      tipo_aula: tipoAula // Garante o tipo de aula
    };

    try {
      if (selectedSlot.data) { // EDIÇÃO
  ...
  const handleDeleteAula = async () => {
    ...
    Alert.alert('Confirmar Exclusão', 'Deseja apagar esta aula?', [
      ...
      { text: 'Deletar', style: 'destructive', onPress: async () => {
        ...
        finally { setIsModalAulaVisible(false); }
      }},
    ]);
  };
  
  // ===== FUNÇÕES DO MODAL DE GRADE ATUALIZADAS =====
  const handleAddHorarioFixo = async () => {
    ...
    try {
      ...
      setNovoHorarioFixo('');
      fetchData(); // Recarrega tudo para mostrar a nova grade
  ...
  const handleDeleteHorarioFixo = async (horarioParaDeletar: string) => {
    try {
      ...
      fetchData(); // Recarrega tudo para mostrar a grade atualizada
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        <View style={styles.headerContainer}>
            ...
            <Button title="Editar Grade" onPress={() => setIsGradeModalVisible(true)} />
        </View>
      ...
      {/* Modal de Edição de AULA (sem alterações visuais) */}
      ...
      {/* Modal de Edição de GRADE (agora conectado à API) */}
            ...
            <View style={styles.modalPage}>
                ...
                <View style={styles.formContainer}>
                    ...
                    <View style={styles.addHorarioContainer}>
                        <MaskInput style={styles.input} value={novoHorarioFixo} onChangeText={setNovoHorarioFixo} mask={[/\d/,/\d/,':',/\d/,/\d/]} placeholder="HH:MM" keyboardType="numeric"/>
...
const styles = StyleSheet.create({
    ...
    slotEmptyContent: { width: '80%', height: 2, backgroundColor: '#e9ecef', borderRadius: 1 },
...
input: { height: 45, width: '100%', borderColor: '#ccc', borderWidth: 1, borderRadius: 4, paddingHorizontal: 10, color: '#000', backgroundColor: '#fff', },
    ...
    modalContainer: { width: '90%', backgroundColor: '#fff', padding: 20, borderRadius: 10, elevation: 5 },
